...
not defined, Zsh will use the user's home directory. Here, `$ZDOTDIR` is set to
`$XDG_CONFIG_HOME/zsh` in `.zshenv`.
...
7.  _`/etc/zlogin`_
8.  _`${ZDOTDIR:-$HOME}/.zlogin`_
9.  _`${ZDOTDIR:-$HOME}/.zlogout`_
10. _`/etc/zlogout`_
...
shell options, and key bindings. It also loads the Homebrew-installed zsh
plugins (autosuggestions, syntax highlighting, history substring search) and
initializes the [Starship][5] prompt.
